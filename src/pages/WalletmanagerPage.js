...
export default function WalletManagementPage() {
  ...
  useEffect(() => {
    // Gọi hàm xử lý callback thanh toán khi trang được tải lại
    handlePaymentCallback();
    fetchTransactions(currentPage, pageSize); // Fetch wallet transactions
  ...
  const fetchTransactions = async (page, pageSize) => {
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        // Lọc giao dịch có transactionId là số lẻ
        const oddTransactions = response.data.filter(transaction => transaction.transactionId % 2 !== 0);
  
        // Đảo ngược mảng để các giao dịch mới nhất hiển thị ở đầu
        const reversedTransactions = oddTransactions.reverse();
  ...
  return (
    <Layout>
      <Card sx={{ minWidth: 275 }}>
        <CardContent>
          <Typography variant="h5" component="div" align="center" color="#0F1E32">
            Quản lý ví
          </Typography>
          <Typography variant="h6" color="text.secondary" align="center">
            Số dư ví: {balance.toLocaleString('vi-VN')} VND
          </Typography>

          <Button
            variant="contained"
            sx={{ marginTop: 2 }}
            onClick={() => setOpenVNPayDialog(true)}
          >
            Nạp tiền qua VNPay
            <VNPayIcon />
          </Button>

          <Dialog open={openVNPayDialog} onClose={() => setOpenVNPayDialog(false)}>
            <DialogTitle>Nhập số tiền nạp</DialogTitle>
            <DialogContent>
              <DialogContentText>
                Vui lòng nhập số tiền bạn muốn nạp vào ví.
              </DialogContentText>
              <TextField
                autoFocus
                margin="dense"
            ...
            <DialogActions>
              <Button onClick={() => setOpenVNPayDialog(false)}>Hủy</Button>
              <Button onClick={handleVNPayTopUp} disabled={loading}>
                {loading ? <CircularProgress size={24} /> : 'Nạp tiền'}
              </Button>
            </DialogActions>
          </Dialog>
        </CardContent>
